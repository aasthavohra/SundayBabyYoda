package com.upskill.assignment1;

public class Assignment_1_2 {

	/*Problem 02: A rectangle width and length are: 9 and 13 inches.
	Write a method to display the perimeter of rectangle in console output. */


		public static void main(String[] args) {
			rectanglePerimeter();
			System.out.println("return type method The perimeter of the rectangle" + rectanglePerimeter());
		}
		
	
		
		public static int rectanglePerimeter(){
			int a = 9;
			int b = 13; 
			
			int perimeter = 2*(a+b);
			System.out.println("rectangle width and length are: 9 and 13 inches" + perimeter); 
			return perimeter;
		}
}




